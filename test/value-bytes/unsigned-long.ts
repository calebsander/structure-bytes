...
export = () => {
	...
	assert.deepEqual(new Uint8Array(type.valueBuffer('0')), new Uint8Array(8).fill(0x00))
	assert.equal(type.readValue(new Uint8Array(8).fill(0x00).buffer), '0')
	assert.deepEqual(new Uint8Array(type.valueBuffer(String(2 ** 32))), new Uint8Array([0, 0, 0, 1, 0, 0, 0, 0]))
	assert.equal(type.readValue(new Uint8Array([0, 0, 0, 1, 0, 0, 0, 0]).buffer), String(2 ** 32))
