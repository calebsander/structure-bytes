...
export = () => {
	...
	assert.deepEqual(new Uint8Array(gb.toBuffer()), concat([bufferFrom([VALUE.length]), VALUE]))
	...
	assert.deepEqual(new Uint8Array(type.readValue(buffer)), new Uint8Array(0))

	assert.throws(
		() => type.readValue(bufferFrom([1]).buffer),
