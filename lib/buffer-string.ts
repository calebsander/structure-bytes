...
export function toString(bytes: ArrayBuffer | Uint8Array) {
	...
	for (let i = 0; i < codePoints.length; i += MAX_ARGUMENTS_LENGTH) {
		string += String.fromCharCode(...codePoints.slice(i, i + MAX_ARGUMENTS_LENGTH))
	}
	return string
}
...
export function toBinaryString(buffer: ArrayBuffer): string {
	...
	for (const byte of castBuffer) string += String.fromCharCode(byte)
	return string
}
