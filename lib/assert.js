const assert = {
	instanceOf: (instance, constructor) => {
		if (!(instance instanceof constructor || (instance !== undefined && instance !== null && instance.constructor === constructor))) { //second case is necessary for JavaScript primitives
			throw new TypeError(String(instance) + ' is not an instance of ' + constructor.name);
		}
	},
	integer: (instance) => {
		assert.instanceOf(instance, Number);
		if (!Number.isSafeInteger(instance)) throw new RangeError(String(instance) + ' is not an integer');
	},
	between: (lower, value, upper, message) => {
		if (value < lower || value >= upper) throw new RangeError((message || '') + (String(value) + ' is not in [' + String(lower) + ',' + String(upper) + ')'));
	},
